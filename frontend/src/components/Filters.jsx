...
const Filters = ({ setMainData }) => {
  ...
  const getDataFromDB = async (year) => {
    try {
      const response = await fetch(`http://localhost:8000/api/data/year/${year}`);
  ...
  return (
    <div className='tabsClass mt-4 mb-4' style={{ display: 'flex', justifyContent: 'center', flexDirection: 'row', alignItems: 'center' }}>
      ...
      <Button variant="danger" onClick={handleReset} style={{ marginLeft: '1vw', width: '10rem' }}>
        Reset Filters
      </Button>
    </div>
  );
